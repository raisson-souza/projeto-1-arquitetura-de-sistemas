...
const seedDb = async () => {
    const seedOrderStatus = (await prisma.paymentOrderStatus.findMany()).length === 0
    ...
    const seedPaymentStatus = (await prisma.paymentStatus.findMany()).length === 0

    if (seedPaymentStatus) {
    ...
    const seedPaymentMethods = (await prisma.paymentMethod.findMany()).length === 0

    if (seedPaymentMethods) {
