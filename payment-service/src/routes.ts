...
export default function BindExpress(app: Express) {
    const port = process.env.PAYMENT_SERVICE_PORT ?? 8002

    app.use(express.json())

    app.get('/', (_, res) => res.send('Projeto 1 - Arquitetura de Sistemas / SERVICE PAYMENTS'))
    ...
    app.listen(port, () => {
        return console.log(`Payments escutando em http://localhost:${port}`)
    })
}
